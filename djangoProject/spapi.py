...
def sp_api_request(api_url, params):
    # 发起请求
    ...
    if response.status_code == 200:
        ...
        # 返回错误信息
        return {'code': response.status_code, 'msg': 'Request failed'}
...
if __name__ == '__main__':
    ...
    # 发起请求
    result = sp_api_request(api_url, params)
    print(result)
...
def test():
    ...
    # Set up your Amazon MWS credentials
    MWS_ACCESS_KEY = 'YOUR_ACCESS_KEY'
    MWS_SECRET_KEY = 'YOUR_SECRET_KEY'
    MWS_SELLER_ID = 'YOUR_SELLER_ID'
    ...
    # Request a report
    report_request = mws.request_report(
        ReportType='_GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2_',
        StartDate='2020-01-01',
        EndDate='2020-01-31'
    )

    # Get the report request id
    report_request_id = report_request['ReportRequestInfo']['ReportRequestId']

    # Check the status of the report
    report_status = mws.get_report_request_list(
        ReportRequestIdList=[report_request_id]
    )

    # Get the report id from the report status
    report_id = mws.get_report_request_list(
        ReportRequestIdList=[report_request_id]
    )['ReportRequestInfo']['GeneratedReportId']

    # Download the report
    report = mws.get_report(
        ReportId=report_id
    )

    # Save the report
    with open('report.csv', 'w+') as file:
        file.write(report['Content'].decode("utf-8"))
